...
if st.button("🚀 AI 상권 분석", type="primary"):
    ...
    if result:
        ...
        # 추천 업종 위치 찾기
        # -------------------------
        recommended_categories = [name for name, count in recommendation["recommend"]]
        ...
        for item in top5:
            st.write(
                ...
                f"- 창업 점수 : {item['점수']}점\n"
                f"- 현재 매장수 : {item['매장수']}개\n"
                f"- 추천 위치 : {item['추천위도']}, {item['추천경도']}"
            )

        st.write("🔥 추천 매장 수:", len(recommended_stores))
        ...
        st_folium(map_obj, width=1200, height=700, returned_objects=[])
        ...
        if stores:
            ...
            for item in scores:
                ...
                st.write(f"{icon} {item['업종']} : {score}점 (현재 {item['매장수']}개)")
